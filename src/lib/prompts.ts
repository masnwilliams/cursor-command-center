...
export function buildHypeshipPrReviewPrompt({
  prUrl,
  repo,
  description,
  files,
  ...
  return `Review this PR incrementally using the /incremental-pr-review skill.
...
## Instructions
1. Use the /incremental-pr-review skill to review this PR
2. Review incrementally — present one chunk at a time, then return so the user can provide comments
3. After each chunk, stop and wait for feedback before continuing
4. Reuse this same worker for subsequent chunks (do not spawn new workers)
5. Return from worker to orchestrator to user after each chunk so the user can respond`;
}
