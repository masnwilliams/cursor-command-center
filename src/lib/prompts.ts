...
export function buildHypeshipReviewPrompt(prUrl: string): string {
  return `Review this PR incrementally: ${prUrl}

Clone the repo containing this PR and use the /incremental-pr-review skill to review it.

## Worker Instructions

- Use a single worker for the entire review
- The worker should clone the repo, check out the PR, and begin the incremental review using /incremental-pr-review
- After each diff chunk is analyzed, return the chunk analysis and review comments back to the orchestrator
- The orchestrator should present each chunk to the user and wait for their feedback before continuing
- Do NOT proceed to the next chunk until the user explicitly says to continue
- When the user provides feedback or review comments for a chunk, pass them back to the worker before continuing to the next chunk

## Flow

1. Spin up 1 worker to handle the review
2. Worker clones the repo and checks out the PR
3. Worker runs /incremental-pr-review which presents the diff in ~100-line chunks
4. After each chunk: worker returns analysis to orchestrator -> orchestrator returns to user -> user provides feedback -> orchestrator sends feedback to same worker -> worker continues to next chunk
5. Reuse the same worker for all chunks — do not spin up new workers for each chunk
6. When all chunks are reviewed, the worker compiles the final review comments and submits via gh CLI (only after user approval)

## Important

- The same worker must be reused across the entire review session
- Each chunk should pause and wait for user input before continuing
- The user controls the pace — never auto-advance through chunks`;
}
